...
export const powerLoraSelector: ComfyUIParameter =
    {
        ...
        getEditorValue({data, entry}): PowerLoraSelectorConfig {
            return {
                ...
                defaultValue: Array.from({length: parseInt(data.get(`count_${entry.entryId}`) as string)},
                    (_, i) => ({
                        lora: data.get(`lora_${entry.entryId}_${i}`) as string,
                        strength: parseFloat(data.get(`lora_strength_${entry.entryId}_${i}`) as string),
                        on: !!data.get(`lora_on_${entry.entryId}_${i}`)
                    })),
            };
        },
        ...
        setInputData({data, entry}, input): void {
            ...
            if (!inputs) return;
            const count = parseInt(data.get(`count_${entry.entryId}`) as string);
            for (let i = 0; i < 10; i++) {
                if (i < count) {
                    inputs[`lora_${i + 1}`] = {
                        lora: data.get(`lora_${entry.entryId}_${i}`) as string,
