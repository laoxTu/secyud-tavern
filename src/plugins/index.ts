...
export class Registry<T extends Registerable> {
  ...
  private sortedItems: T[] | null = null;
  ...
  record(name?: string): T | null {
    return (name ? this.records[name] : this.sorted()[0]) ?? null;
  }
  ...
  sorted(): T[] {
    if (this.sortedItems === null) {
      this.sortedItems = this.sortByRequires();
    }
  ...
  protected sortByRequires(): T[] {
    ...
    for (const record of recordList) {
      ...
      for (const dep of deps) {
        ...
        if (!records[dep]) {
          throw new Error(
            `[Sort Error] Component "${id}" depends on non-existent component "${dep}".`,
          );
        }
...
export function getRegistry<T extends Registerable>(
  name: string,
  init?: (registry: Registry<T>) => void,
): Registry<T> {
  ...
  if (!g.__registries.has(name)) {
    ...
    init?.(registry);
    g.__registries.set(name, registry);
  }
