...
describe('extractVariableChanges', () => {
    it('空字符串时清空 variables 和 content', () => {
        ...
        extractVariableChanges(history, '');
    ...
    it('未传 text 时清空 variables 和 content', () => {
        const history = makeHistory();
        extractVariableChanges(history, undefined);
        expect(history.variables).toEqual([]);
        expect(history.content).toBe('');
    });
    ...
    it('没有 variable_changes 标签时 variables 为空, content 保留去除首尾空白后的文本', () => {
        const history = makeHistory();
        extractVariableChanges(history, '  这是一句普通的话。  ');
        expect(history.variables).toEqual([]);
    ...
    it('单个对象块被解析为变更项', () => {
        const history = makeHistory();
        extractVariableChanges(history, '回复。<variable_changes>{"op":"update","path":"/player/hp","value":80}</variable_changes>');
    ...
    it('数组块中的多个合法项全部被解析', () => {
        ...
        extractVariableChanges(history, text);
        expect(history.variables).toEqual([
    ...
    it('缺少 value 的项只要 op 和 path 合法也会保留', () => {
        const history = makeHistory();
        extractVariableChanges(history, '<variable_changes>{"op":"remove","path":"/player/buff"}</variable_changes>');
    ...
    const userChanges = [
        ...
        {op: 'add', path: 'story/characters/stranger', value: {name: '未知', appearance: '陌生面孔', action: '敲门问候'}},
    ];
